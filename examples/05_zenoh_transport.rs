...
async fn main() -> mini_ros::error::Result<()> {
    ...
    let position_pub = node
        .create_publisher::<Float64Msg>("/robot/position/x")
        .await?;
    let temperature_pub = node
        .create_publisher::<Float64Msg>("/sensors/temperature")
        .await?;
    let status_pub = node
        .create_publisher::<StringMsg>("/telemetry/status")
        .await?;

    // Create a subscriber to demonstrate pub/sub capabilities
    let position_sub = node
        .create_subscriber::<Float64Msg>("/robot/position/x")
        .await?;
    ...
    for i in 0..15 {
        ...
        position_pub
            .publish(&Float64Msg { data: x_position })
            .await?;
        ...
        if i % 3 == 0 {
            ...
            temperature_pub
                .publish(&Float64Msg { data: temperature })
                .await?;
            info!(
                "🌡️  [TRANSPORT] Published temperature: {:.2}°C",
                temperature
            );
        }
