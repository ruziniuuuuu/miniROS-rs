...
use mini_ros::visualization::{
    VisualizationClient, VisualizationConfig, RobotPose, PointCloud, LaserScan, Visualizable
};
...
async fn main() -> mini_ros::error::Result<()> {
    ...
    for i in 0..20 {
        ...
        if i % 3 == 0 {  // Update every 3rd frame for performance
            ...
            let point_cloud = PointCloud { points: point_cloud_data };
            point_cloud.visualize(&viz_client, "sensors/lidar/points")?;
        }

        // === Laser Scan (2D) ===
        if i % 2 == 0 {  // Update every 2nd frame
        ...
        info!("🤖 Step {}: Robot at ({:.2}, {:.2}, {:.2}), Battery: {:.1}%, CPU: {:.0}%", 
              i + 1, x, y, z, battery, cpu_load);
