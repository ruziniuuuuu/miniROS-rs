...
use mini_ros::types::{
    Header, MiniRosMessage, OdometryMessage, Point3D, Quaternion, TwistMessage, Vector3,
};
...
impl TurtlebotController {
    ...
    pub async fn run_demo(&mut self) -> Result<()> {
        ...
        // 1. Move forward
        self.execute_pattern(MovementPattern::Forward(0.5), 3.0)
            .await?;
        sleep(Duration::from_secs(1)).await;

        // 2. Circle motion
        self.execute_pattern(MovementPattern::Circle(0.3, 0.5), 5.0)
            .await?;
        sleep(Duration::from_secs(1)).await;

        // 3. Square pattern
        self.execute_pattern(MovementPattern::Square(2.0), 10.0)
            .await?;
        sleep(Duration::from_secs(1)).await;

        // 4. Spin in place
        self.execute_pattern(MovementPattern::TurnLeft(1.0), 3.0)
            .await?;
    ...
    async fn publish_mock_odometry(&mut self) -> Result<()> {
        ...
        let odom_pub = self
            .node
            .create_publisher::<OdometryMessage>("/odom")
            .await?;
